...
static CellWidget*** allocateCellsWidgets(BitSweep* const bitSweep)
{
    ...
    const float gameAreaWidth = COLS * CELL_SIZE;
    const float size = (gameAreaWidth - (width - 1) * margin - margin * 2) / width;
    ...
    for (int i = 0; i < width; i++)
    {
        ...
        for (int j = 0; j < height; j++)
        {
            float x = i * (size + margin) + margin + GAME_AREA_MARGIN;
            float y = j * (size + margin) + margin + GAME_AREA_MARGIN;
...
void handleBitSweepWidgetInput(BitSweepWidget* const widget)
{
    if (!widget || !widget->bitSweep || !widget->cellsWidgets) return;

    const unsigned width = bitSweepWidth(widget->bitSweep);
    const unsigned height = bitSweepHeight(widget->bitSweep);

    for (unsigned i = 0; i < width; i++)
    {
        for (unsigned j = 0; j < height; j++)
        {
            CellWidget* cellWidget = widget->cellsWidgets[i][j];
            if (cellWidget) {
                handleCellWidgetInput(cellWidget);
            }
        }
    }
}