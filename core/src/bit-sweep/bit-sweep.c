...
bool bitSweepIsWon(const BitSweep* const bitSweep)
{
    if (!bitSweepIsFinished(bitSweep)) {
        return false;
    }

    // Check if all non-bomb cells are opened
    int openedCells = bitSweepOpenedCellsCount(bitSweep);
    int nonBombCells = bitSweepNonBombCellsCount(bitSweep);

    return openedCells == nonBombCells;
}
...
Cell* openCellAt(BitSweep* const bitSweep, const int i, const int j)
{
    ...
    if (cellIsOpened(cell) || bitSweep->isFinished || cellIsFlagged(cell))
        return cell;
